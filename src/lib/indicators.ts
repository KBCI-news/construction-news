...
const RULES: Rule[] = [
  {
    key: "base_rate",
    // 실제 "결정된 수준"만 인정한다. "…%로" 형태 + 결정 동사를 함께 요구하며,
    // 이렇게 해야 "0.25%포인트 인상" 같은 변동폭이 수준으로 오독되지 않는다.
    // 그래야 "종착지는 연 3.5%?", "연말 기준금리 3% 전망" 같은 전망치를 거른다.
    // 변동폭("0.25%포인트")도 어느 패턴에도 걸리지 않는다.
    patterns: [
      /기준금리.{0,40}?(\d(?:\.\d{1,2})?)\s*%\s*(?:로|으로)\s*.{0,12}?(?:동결|인상|인하|결정|유지|올렸|내렸|낮췄)/,
    ],
    // 한국은행 맥락일 때만 인정
    require: /(한국은행|한은|금융통화위원회|금통위)/,
    // 전망·관측 기사는 현재 수준이 아니다
    exclude: new RegExp(
      `${FOREIGN.source}|전망|예상|관측|종착지|시나리오|컨센서스|베팅|점쳐`,
      "i",
    ),
    min: 0.25,
    max: 6,
