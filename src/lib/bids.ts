...
export type BidAreaId = "collection" | "scan" | "archive" | "lease" | "rights";
...
export const BID_AREAS: BidArea[] = [
  {
    id: "collection",
    label: "채권추심",
    definition: "채권추심·미수금 회수 위탁, 체납채권 정리",
...
const RULES: Record<BidAreaId, AreaRule> = {
  collection: {
    strong: [
      "채권추심", "채권 추심", "추심용역", "추심 용역", "추심 위탁",
      "채권 회수 용역", "채권회수 용역", "채권회수용역",
      "미수금 회수", "미수채권", "미수 채권", "미수금 채권",
      "체납채권", "체납 채권", "체납액 징수", "체납액 정리",
      "구상채권", "구상금 채권", "부실채권 회수", "연체채권 회수",
    ],
    weak: ["추심", "미수금", "채권회수", "채권 회수", "체납액"],
    context: ["회수", "위탁", "용역", "징수", "정리", "관리"],
    // '채권'은 회사채·국공채와 정면충돌하므로 weak에 넣지 않는다.
    // 금융시장 채권 공고가 섞여 들면 표 전체의 신뢰가 무너진다.
    deny: [
      "회사채", "국채", "지방채", "공사채", "전환사채", "채권 발행",
      "채권형", "채권시장", "채권평가", "채권운용",
    ],
    classes: [],
  },
  scan: {
    ...
    hardDeny: [
      ...
      "인공지능", "AI 학습", "실증모델",
      // "위치기반 도로점용허가관리시스템 및 DB구축" — 분류(DB구축)만 같은 GIS 사업
      "위치기반", "도로점용",
    ],
  ...
  lease: {
    strong: [
      ...
      "확정일자", "임차권등기",
      // LH·지자체 전세임대주택의 물색·권리분석·계약 위탁 공고.
      // 단독 "임차용역"은 절대 넣지 않는다 — 실측 24시간치 25건 전원이
      // 제설장비·구급차·통근버스 임차였다. 주택 맥락이 붙은 형태만 확정한다.
      "전세임대 임차", "기존주택 전세임대", "전세임대주택",
      "주택 임차용역", "임대차조사업무",
    ],
    // '전대'는 무단전대·불법전대(strong)로 충분하다 — weak로 두면
    // '사전대응' 류의 무관 공고까지 끌어들였다
    weak: ["임대차", "임차인", "전세", "보증금", "월세", "주택임대차"],
    context: [
      "조사", "실태", "점검", "확인", "검증", "현황", "실사", "열람",
      "분석", "컨설팅", "피해", "지원",
    ],
    deny: ["전세계", "청사", "관사", "사택", "임차료 산정", "감정평가 수수료", "임대료 인상"],
    // 실측 오탐: "전세버스 임차"가 weak '전세'를 타고 들어왔다. 차량·장비
    // 임차는 표현이 무엇이든 우리 사업이 아니다 — strong '임차 용역'까지 차단.
    // ('버스'는 2자 term이라 어절 머리에서만 걸린다 — '전세버스'는 명시해야 잡힌다)
    hardDeny: [
      "전세버스", "관광버스", "통학버스", "셔틀버스", "통근버스", "버스",
      "차량", "승합", "렌터카", "리스", "구급차", "제설", "덤프", "굴착기",
      "중장비", "장비 임차", "기기 임차", "사무기기", "복합기", "모듈러",
      "청사 임차", "사무실 임차", "연수시설", "소프트웨어", "선박", "항공기",
      "수학여행", "현장체험학습",
    ],
    classes: [],
  },
