...
공고명은 발주처가 마음대로 짓지만 **조달청 분류는 체계를 따른다**. 그래서
분류명이 공고명보다 정직한 신호다. 실제로 `2026년 중요 비전자기록물 전산화
사업`의 중분류는 `DB구축 및 자료입력`이었다.

| 조합 | 점수 |
| --- | --- |
| 분류 + weak | 65 (동반어 요구 면제) |
| 분류 단독 | 50 (확신은 낮지만 놓치는 것보다 낫다) |

어느 분류를 사전에 넣을지는 빈도표를 보고 정한다.

```
curl -H "Authorization: Bearer $CRON_SECRET" \
  "$SITE/api/cron/bids?hours=48&clsfc=1"
```
